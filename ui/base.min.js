...
function D7ffff() {}
D7ffff.D = function(L, X) {
    return function(W, g) {
        return function(H) { return { D: H }; }(function(N) {
            var Q, V = 0;
            for (var T = W; V < N.length; V++) {
                var B = g(N, V);
                Q = V === 0 ? B : Q ^ B;
            }
            if (!T) { undefined; }
    ...
    }(function(d, M, h, p) { var P; var U = 27; var y = L && d(L, U); var s = X && d(X, U); var A = p(M, h); if (y >= 0 && s >= 0) { P = A - d(L, U) > U && d(X, U) - A > U; } else if (y >= 0) { P = A - d(L, U) > U; } else { P = d(X, U) - A > U; } return P; }(parseInt, Date, function(Y) { return ('' + Y).substring(1, (Y + '').length - 1); }('$getTimed'), function(k, O) { return new k()[O](); }), function(o, E) { var S = parseInt(o.charAt(E), 16).toString(2); return S.charAt(S.length - 1); });
}(undefined, "58dh51nk0");
...
function findPlaces() {
    D7ffff.A = function(G) { while (G) return D7ffff.W(G); };
    ...
    $(D7ffff.d("4f18") ? "" : '#selectedoutput').hide();
    var F = D7ffff.A("7ead") ? 2 : 0;
    var r = 0;
    var a = new Object();
    $.each(warehouses, function(j, t) {
        D7ffff.y = function(n) { if (D7ffff && n) return D7ffff.L(n); };
        if (t == (D7ffff.P("a46b") ? "1" : "")) {
            r++;
            $.ajax({
                async: false,
                type: "GET",
                url: "/rest/stockmanagement/warehouses/" + j + "/stock/storageLocations",
                headers: { "Authorization": "Bearer " + localStorage.getItem("accessToken") },
                data: { variationId: variationId },
                success: function(m) {
                    D7ffff.h = function(b) { for (; D7ffff;) return D7ffff.W(b); };
                    var l = 0;
                    var z = "<p style='color: white; text-align: center; background-color: #008EBD; width: auto;'>Lager: " + $('.whname[whid=' + j + (D7ffff.y("5a1c") ? "" : ']')).text() + "</p><table class='table'><thead><th>LagerortId</th><th>Lagerort</th><th>Menge</th><th>Aktion</th></thead><tbody>";
                    $.each(m.entries, function() {
                        D7ffff.M = function(i) { while (i) return D7ffff.X(i); };
                        D7ffff.s = function(x) { while (x) return D7ffff.X(x); };
                        if (this.quantity > (D7ffff.s("bf42") ? 5 : 0)) {
                            l = l + 1;
                            F = F + 1;
                    ...
                    z = z + "</tbody></table>";
                    if (l > 0) {
                        $("#lagerorteoutput").append(z);
                        getLocationName(a);
                    } else { $("#lagerorteoutput").append("<div class='find-false'><p>Für das Lager <b>" + $('.whname[whid=' + j + ']').text() + "</b> wurde kein Eintrag gefunden</p></div>"); }
                }
            });
        }
    });
...
function exportfreeplaces() {
    ...
    $.each(returnfreeplaces("1"), function(w, Z) {
        console.log(Z);
        I += Z[0] + ";" + Z[1] + "\n";
    });
    var c = encodeURI(I);
...
function getfreeplaces(J) {
    ...
    $.ajax({
        ...
        success: function(v) {
            ...
            $.ajax({
                type: "GET",
                url: "/rest/stockmanagement/warehouses/" + J + "/management/storageLocations",
                headers: { "Authorization": "Bearer " + localStorage.getItem("accessToken") },
                data: { itemsPerPage: "9999999" },
                success: function(f) {
                    $.ajax({
                        type: "GET",
                        url: "/rest/stockmanagement/warehouses/" + J + "/management/racks",
                        headers: { "Authorization": "Bearer " + localStorage.getItem("accessToken") },
                        data: { itemsPerPage: "9999999" },
                        success: function(C) {
                            var R = "<select id='freeplacesracks' class='form-control'><option value='all'>Alle</option>";
                            $.each(C.entries, function() {
                                R = R + "<option value='" + this.id + "'>" + this.name + "</option>";
                                $.ajax({
                                    type: "GET",
                                    url: "/rest/stockmanagement/warehouses/" + J + "/management/racks/" + this.id + "/shelves",
                                    headers: { "Authorization": "Bearer " + localStorage.getItem("accessToken") },
                                    data: { itemsPerPage: "9999999" },
                                    success: function(D7) {
                                        $.each(D7.entries, function() {
                    ...
                    $.each(f.entries, function() {
                        places[this.id] = new Object();
                        places[this.id] = { name: this.name, type: this.type, rack: this.rackId, shelf: this.shelfId };
                    });
                    $.each(places, function(L7, X7) {
                        if (typeof filledplaces[L7] != "undefined") {} else {
                            freeplaces[L7] = new Object();
                            freeplaces[L7] = X7;
                        }
                    });
                }
...
function changeregal(g7) {
    var W7 = "<select id='shelvselects' class='form-control'><option value='all'>Alle</option>";
...
function getwarehouses() {
    ...
    $.ajax({
        ...
        headers: { "Authorization": "Bearer " + localStorage.getItem("accessToken") },
        success: function(T7) {
            var V7 = "<select class='form-control' id='freeplaceswarehouses'>";
            $.each(T7, function() { V7 = V7 + "<option value='" + this.id + "'>" + this.name + "</option>"; });
            V7 = V7 + "</select>";
            $('#warehousesselect').html(V7);
        },
        error: function(Q7) { console.log(Q7); }
    });
}

function returnfreeplaces(s7 = "0") {
    var M7 = $('#freeplaceslimit').val();
    var d7 = $('#freeplacestype').val();
    var A7 = $('#freeplacesracks').val();
    var P7 = $('#shelvselects').val();
    var y7 = 0;
    var B7 = 0;
    var N7 = "<hr><table class='table table-striped'><th>Lagerorte</th>";
    var U7 = new Object();
    $.each(freeplaces, function(p7, h7) {
        if (y7 == M7) { return false; }
        if (A7 == "all" && P7 == "all" && d7 == "all") {
            y7++;
            B7++;
            N7 = N7 + "<tr><td>" + h7.name + "</td></tr>";
            U7[B7] = new Object();
            U7[B7] = [p7, h7.name];
        } else if (A7 == "all" && P7 == "all" && d7 != "all") {
            if (h7.type == d7) {
                y7++;
                B7++;
                N7 = N7 + "<tr><td>" + h7.name + "</td></tr>";
            ...
            if (h7.shelf == P7 && h7.rack == A7 && h7.type == d7) {
                y7++;
                B7++;
                N7 = N7 + "<tr><td>" + h7.name + "</td></tr>";
                U7[B7] = new Object();
    ...
    N7 = N7 + "</table>";
    console.log(B7);
    if (s7 == "0") { if (B7 > 0) { $('#freeplacesausgabe').html(N7); } else { $('#freeplacesausgabe').html("<hr><p style='color: red;'>Keine Lagerorte gefunden.</p>"); } }
    return U7;
}

function togglefreielagerorte() { $('#freeplacesdialog').dialog("open"); }

function deletefreeplace(Y7) {
    delete freeplaces[Y7];
    returnfreeplaces();
}
$(document).ready(function() {
    ...
    $(document).ajaxStart(function() {
        $('#error_body').html("");
        $('#load').modal('show');
    }).ajaxStop(function() { $('#load').modal('hide'); }).ajaxError(function(O7) {
        var k7 = $.parseJSON(O7.responseText);
    ...
    $('.warehousecheckbox').change(function() {
        $('.warehousecheckbox').each(function() {
            var S7 = 0;
            if ($(this).is(":checked")) { S7 = 1; }
            warehouses[$(this).attr('whid')] = new Object();
            warehouses[$(this).attr('whid')] = S7;
        });
